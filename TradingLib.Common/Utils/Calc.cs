...
namespace TradingLib.Common
{
    ...
    public static class Calc
    {
        ...
        static int CompareListBigDecimal(decimal _a, decimal _b)//????
        {
            decimal _temp = _a - _b;
            if (_temp < 0) return 1;
            if (_temp > 0) return -1;
        ...
        static int CompareListSmallDecimal(decimal _a, decimal _b)//???????
        {
            decimal _temp = _a - _b;
            if (_temp > 0) return 1;
            if (_temp < 0) return -1;
        ...
        //    if (positions != null)
        //    {
        //        return positions.Sum(e => e.ClosedPL * e.oSymbol.Multiple);//?????????? = ??????????*????
        //    }
        //    return 0;
        //}
        ...
        //    if (positions != null)
        //    {
        //        return positions.Sum(e => e.UnRealizedPL * e.oSymbol.Multiple);//?????????? = ???????????*????
        //    }
        //    return 0;
        //}
        ...
        //            int i = 0;
        //        }
        //        return positions.Sum(e => e.SettleUnrealizedPL * e.oSymbol.Multiple);//?????????? = ???????????*????
        //    }
        //    return 0;
        //}
        ...
        //    {
        //        return p.UnsignedSize * (p.oSymbol.Margin + (p.oSymbol.ExtraMargin > 0 ? p.oSymbol.ExtraMargin : 0));//??????????????????????????
        //    }
        ...
        //    //{
        //    //    return pos.UnsignedSize * pos.AvgPrice * pos.oSymbol.ULSymbol.Multiple;//??????õ?????????
        //    //}
        //    //else//???????????,????????????????????
        //    //{
        ...
        public static decimal CalFundRequired(Symbol symbol, decimal price=0,int size=1)
        {
            ...
            else if (symbol.SecurityType == SecurityType.OPT)//?????????????
            {
                return price * symbol.Multiple * size;
            }
            //else if (symbol.SecurityType == SecurityType.INNOV)//????????????
            //{
            //    if (symbol.Margin > 0)
            //    {
